...
def shutdown_event():
    ...
    if os.path.exists(TEMP_UPLOAD_DIR):
        try:
            ...
            print(
                f"Error cleaning up temporary upload directory {TEMP_UPLOAD_DIR}: {e}"
            )
    ...
    try:
        ...
        try:
            ...
            ingestion_schema_id = get_ingestion_logic_schema_id(
                conn, INGESTION_LOGIC_VERSION_NUMBER
            )
            if ingestion_schema_id is None:
                ...
                raise HTTPException(
                    status_code=500,
                    detail=f"Ingestion logic version {INGESTION_LOGIC_VERSION_NUMBER} not found in database. Setup may be incomplete.",
                )
            ...
            success = await run_in_threadpool(
                process_xml_file, conn, temp_file_path, ingestion_schema_id
            )
            ...
            if success:
                ...
                raise HTTPException(
                    status_code=500,
                    detail="Failed to process XML file. Check server logs for details.",
                )
            ...
            if (
                temp_file_path and os.path.exists(temp_file_path) and not success
            ):  # Only remove if not successful, as success implies archiving
                try:
        ...
        print(
            f"An unexpected error occurred during XML ingestion: {e}"
        ...
        if temp_file_path and os.path.exists(temp_file_path):
            try:
                ...
                print(
                    f"Error cleaning up temporary file {temp_file_path} after an error: {cleanup_e}"
                )
        raise HTTPException(
